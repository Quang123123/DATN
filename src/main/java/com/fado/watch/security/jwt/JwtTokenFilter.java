    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if(token!=null &&jwtProvider.validateToken(token)){ 
                ...
                String authority = jwtProvider.getAuthorityFromToken(token);

                UserDetails userDetails;
                if (authority.equals("ROLE_CUSTOMER")){
                     userDetails = customerUserDetailService.loadUserByUsername(username);
                }else {
                    userDetails = staffUserDetailService.loadUserByUsername(username);
                }
